...
DEFAULT_OUTPUT_PATH = os.path.abspath( os.path.join(os.path.dirname(__file__), r'../output') )
...
class ResultWriter:
    ...
    def __call__(self, result: dict, audio_path: Union[str, None, TextIO], options: dict):
        ...
        # if it was instantiated without an output_dir, just print to console instead
        else:
            self.write_result(result, file=None, options=options)

    def write_result(self, result: dict, file: Union[TextIO, None], options: dict):
...
class WriteSummary(ResultWriter):
    extension: str = "_summary.txt"

    def write_result(self, result: str, file: TextIO, options: dict):
        print(result, file=file, flush=True)


def get_writer(
    output_format: str, output_dir: Union[str, None]
